...
function ToolboxItem({ entry }: { entry: ComponentRegistryEntry }) {
  ...
  return (
    <button
      ...
      title={t(`builder.components.${entry.type}.description`)}
    >
      ...
      <span className="min-w-0">
        <span className="block truncate text-[12px] font-medium text-[hsl(var(--foreground))]">{t(`builder.components.${entry.type}.label`)}</span>
        <span className="block truncate text-[10px] text-[hsl(var(--muted-foreground))]">{t(`builder.components.${entry.type}.description`)}</span>
      </span>
    </button>
  )
}
  ...
  return (
    <div className="flex h-full w-64 shrink-0 flex-col border-r border-[hsl(var(--border))] bg-[hsl(var(--card))]">
      <div className="border-b border-[hsl(var(--border))] p-3">
        <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">{t('builder.components.title')}</p>
        <div className="relative">
          <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" />
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('builder.components.search')} className="h-8 pl-7 text-xs" />
        </div>
      </div>
        ...
        <div className="space-y-4 p-3">
          {COMPONENT_CATEGORIES.map((cat) => {
            const items = Object.values(COMPONENT_REGISTRY).filter(
              (c) => c.category === cat && (!q || t(`builder.components.${c.type}.label`).toLowerCase().includes(q) || t(`builder.components.${c.type}.description`).toLowerCase().includes(q)),
            )
            if (items.length === 0) return null
            return (
              <div key={cat}>
                <p className="mb-1.5 px-1 text-[10px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">{t(`builder.components.${cat.toLowerCase()}`)}</p>

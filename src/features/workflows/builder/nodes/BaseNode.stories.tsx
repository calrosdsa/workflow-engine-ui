...
import { AlertCircle, CheckCircle2, Database, GripVertical, Loader2, Variable } from 'lucide-react'
...
type NodePreviewState = 'default' | 'hover' | 'focus' | 'active' | 'disabled' | 'loading' | 'error' | 'success'

const previewStates: NodePreviewState[] = ['default', 'hover', 'focus', 'active', 'disabled', 'loading', 'error', 'success']
...
function NodePreview({ state }: { state: NodePreviewState }) {
  const status = state === 'loading'
    ...
    : state === 'error'
      ? { className: 'workflow-node-status--failed', label: 'Failed', icon: <AlertCircle size={10} strokeWidth={3} /> }
      : state === 'success'
        ? { className: 'workflow-node-status--completed', label: 'Completed', icon: <CheckCircle2 size={10} strokeWidth={3} /> }
        : null
    ...
    <div>
      ...
      >
        ...
        <div className="workflow-node-header flex items-center gap-2 px-2.5 py-2">
          ...
          <span className="workflow-node-icon flex h-7 w-7 items-center justify-center rounded-md" aria-hidden="true">
        ...
        <div className="workflow-node-body px-3 py-2.5">
          <div className="flex items-center gap-1 text-[10px]">
            <code className="rounded bg-[hsl(var(--primary))]/10 px-1 py-0.5 font-semibold text-[hsl(var(--primary))]">single</code>
            <span className="text-[hsl(var(--muted-foreground))]">· 2 filters</span>
          </div>
          <div className="mt-1 flex items-center gap-1 text-[10px]">
            <Variable size={11} className="text-[hsl(var(--muted-foreground))]" />
            <code className="font-semibold text-[hsl(var(--primary))]">customer</code>
          </div>
        </div>
        <span className="workflow-node-handle workflow-node-handle--output absolute -right-1.5 top-1/2 -translate-y-1/2" />
        {status && (
          <span className={`workflow-node-status absolute bottom-2 right-2 flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${status.className}`}>
...
export const States = () => (
  <div className="min-h-screen bg-[hsl(var(--background))] p-8 text-[hsl(var(--foreground))]">
    <div className="max-w-3xl">
      ...
      <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">The same compact graph object across interaction and execution states.</p>
      <div

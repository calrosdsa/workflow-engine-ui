...
//
//   kind      where the implementation lives: core (compiled into the engine),
//             connector (a separate gRPC process), template (declarative data
//             interpreted in-process). This was INFERRED — "not in
//             NODE_REGISTRY, so it must be a connector" — which is why a third
//             source of node types had nowhere to go.
//
...
// SERVED. This module fetches it. What stays compiled in is presentation —
// icon, colour, config form — because that genuinely cannot be served.
//
...
/** Provenance. Mirrors graph.NodeKind. */
export type NodeKind = 'core' | 'connector' | 'template'
...
/** One node type reduced to its grouping — no config schema, by design. */
 ...
 *  need no isLoading branch — the same reasoning connector-hooks uses for its
 *  own stable empty array. */
...
/** One palette entry, whatever its provenance. */
 ...
 *  ships with no members today, for declarative node templates) without
 ...
 *  function means a connector now shares a tab with built-ins, but it can

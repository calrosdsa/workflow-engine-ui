    ...
    const view = new EditorView({
      state: EditorState.create({
        ...
        extensions: [
          ...
          EditorView.theme({
            '&': { fontSize: '13px', borderRadius: '10px', border: '1px solid #e2e8f0', backgroundColor: '#fff', overflow: 'hidden' },
            '.cm-content': { fontFamily: "'Fira Mono', 'Cascadia Code', 'Consolas', monospace", padding: '12px 4px', minHeight: '160px', caretColor: '#6366f1' },
            ...
            '.cm-tooltip': { border: '1px solid #e2e8f0', borderRadius: '8px', background: '#fff', boxShadow: '0 8px 24px rgba(15,23,42,0.12)', overflow: 'hidden' },
            '.cm-tooltip-autocomplete ul li[aria-selected]': { background: '#eef2ff', color: '#4338ca' },
            '.cm-tooltip-autocomplete ul li': { padding: '3px 8px', fontFamily: "'Fira Mono', monospace", fontSize: '12px' },
            '.cm-completionDetail': { color: '#94a3b8', fontStyle: 'normal', marginLeft: '8px' },
            '.cm-diagnostic-error': { borderLeft: '3px solid #ef4444' },
            '.cm-lintRange-error': { backgroundImage: 'none', borderBottom: '2px wavy #ef4444', textDecoration: 'underline wavy #ef4444' },
          }),
        ],
      }),
      ...
      <DialogContent className="flex h-[82vh] w-[960px] max-w-[95vw] flex-col overflow-hidden p-0">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Expression Editor
            {label && <span className="text-sm font-normal text-slate-400">— {label}</span>}
          </DialogTitle>
        </DialogHeader>

        <div className="flex min-h-0 flex-1">
          {/* Left: editor + status + preview */}
          <div className="flex min-h-0 flex-1 flex-col gap-3 border-r border-slate-100 p-4">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Expression</span>
            ...
            <div className="rounded-lg bg-slate-50 p-3 text-[11px] leading-relaxed text-slate-500">
              <p className="mb-1 font-semibold text-slate-600">Quick reference</p>
              <p><code className="text-indigo-600">Vars["name"]</code> — workflow variable · <code className="text-indigo-600">Times["name"]</code> — date variable</p>
              <p><code className="text-indigo-600">NodeOutputs["id"]["field"]</code> — previous node output</p>
              <p className="mt-1 text-slate-400">Type to autocomplete · <kbd className="rounded border border-slate-200 bg-white px-1">Ctrl</kbd>+<kbd className="rounded border border-slate-200 bg-white px-1">Space</kbd> to trigger</p>
            </div>
          </div>
                  ...
                  {['All', ...FUNCTION_CATEGORIES].map((cat) => (
                    <button
                      ...
                      className={cn(
                        ...
                        fnCategory === cat ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200',
                      )}
                    >
                      {cat}
                    </button>
...
function StatusBadge({ state }: { state: ValidationState }) {
  if (state.status === 'checking') {
    return <span className="flex items-center gap-1 text-[11px] text-slate-400"><Loader2 size={11} className="animate-spin" />Checking…</span>
  }
  if (state.status === 'valid') {
    return <span className="flex items-center gap-1 text-[11px] font-medium text-emerald-600"><CircleCheck size={11} />Valid</span>
  }
  if (state.status === 'invalid') {
    return <span className="flex items-center gap-1 text-[11px] font-medium text-red-500"><CircleAlert size={11} />Invalid</span>
  }
  return null
}
...
function PreviewPanel({ state }: { state: ValidationState }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50/60 px-3 py-2">
      <Eye size={13} className="shrink-0 text-emerald-600" />
      <span className="text-[10px] font-semibold uppercase tracking-wider text-emerald-700">Preview</span>
      <code className="min-w-0 flex-1 truncate font-mono text-[12px] text-slate-700">{formatPreviewValue(preview.value)}</code>
      <span className={cn('shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium', TYPE_COLORS[preview.type] ?? 'bg-slate-100 text-slate-600')}>
        {preview.type}
      </span>
    </div>
  )
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center gap-1.5 px-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
        {icon}
        {title}
        <span className="rounded-full bg-slate-100 px-1.5 text-[9px] text-slate-400">{count}</span>
      </div>
...
function InsertRow({ mono, sub, badge, onClick }: { mono: string; sub: string; badge?: React.ReactNode; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group flex w-full items-center justify-between rounded-lg border border-transparent px-2.5 py-1.5 text-left transition-colors hover:border-indigo-100 hover:bg-indigo-50"
    >
      <div className="min-w-0">
        <p className="truncate font-mono text-[12px] font-medium text-slate-700">{mono}</p>
...
function NodeContextTree({ schema, search, onInsert }: {
  ...
  return (
    <div className="rounded-lg border border-slate-100">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center gap-1.5 rounded-t-lg bg-slate-50 px-2 py-1.5 text-left"
      >
        {open ? <ChevronDown size={11} className="text-slate-400" /> : <ChevronRight size={11} className="text-slate-400" />}
        <Database size={11} className="text-rose-400" />
        <span className="truncate text-[11px] font-semibold text-slate-600">{schema.nodeLabel}</span>
        <span className="ml-auto rounded bg-slate-200 px-1 text-[9px] text-slate-500">{schema.nodeType}</span>
      </button>
      {open && (
        <div className="p-1">
          {schema.fields.map((f) => (
  ...
  return (
    <div>
      <div className="flex items-center" style={{ paddingLeft: depth * 10 }}>
        {hasChildren ? (
          <button onClick={() => setOpen((o) => !o)} className="flex h-5 w-5 items-center justify-center text-slate-400 hover:text-slate-600">
...
function FunctionRow({ fn, onClick }: { fn: ExprFunction; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group w-full rounded-lg border border-transparent px-2.5 py-2 text-left transition-colors hover:border-indigo-100 hover:bg-indigo-50"
    >
      <div className="flex items-center justify-between gap-2">
        <p className="truncate font-mono text-[12px] font-semibold text-slate-700">{fn.signature}</p>
        <span className="shrink-0 rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium text-slate-500">{fn.category}</span>
      </div>
      <p className="mt-0.5 text-[10px] text-slate-500">{fn.description}</p>
      <p className="mt-0.5 truncate font-mono text-[9px] text-indigo-400 opacity-0 transition-opacity group-hover:opacity-100">{fn.example}</p>
    </button>
  )
}
...
function TypeBadge({ type }: { type: string }) {
  return <span className={cn('rounded px-1.5 py-0.5 text-[10px] font-medium', TYPE_COLORS[type] ?? 'bg-slate-100 text-slate-600')}>{type}</span>
}
